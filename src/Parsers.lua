...
local Parsers = {
  ...
  generateParser('function', function(str)
    ...
    return '';
  end, function(str) return function() error('SECURITY: Cannot parse functions.', 2) end; end); --
  -- ANCHOR userdata
  generateParser('UserData', function() return {}; end, function()
    warn(
        'Cannot encode UserDatas - Try adding a parser/encoder to Conglomeration for this specific type. Encoding as an empty table.')
    return '';
  end); -- !SECTION
