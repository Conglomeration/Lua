--- TODO: Re-code the encoders/serializers
--- Add propper CFrame support that doesn't use jsonencoding
...
local function getOrientId( cf )
  if not isAxisAligned(cf) then return -1 end
  ...
  if not legalOrientId(orientId) then return -1 end
...
local function toQuaternion( cf )
  ...
  local m11, m12, m13, m21, m22, m23, m31, m32,
    m33 = select(4, cf:GetComponents())
...
function CFrameSerializer:DecodeCFrame(
  ...
  local serialCF = raw and cfJson or
                     HttpService:JSONDecode(cfJson)
