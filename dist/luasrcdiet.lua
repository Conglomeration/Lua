...
local requireLibs = {
  equiv = function()
    ...
    function M.source(z, dat)
      ...
      local sh1 = z:match('^(#[^\r\n]*)')
      local sh2 = dat:match('^(#[^\r\n]*)')
      if sh1 or sh2 then if not sh1 or not sh2 or sh1 ~= sh2 then bork('shbang lines different') end end
    ...
    function M.binary(z, dat)
      ...
      local function getint_b(c)
        local n = 0
        if not ensure(c, sz_int) then return end
        for _ = 1, sz_int do n = n * 256 + getbyte(c) end
        return n
      end
      ...
      local function getsizet_b(c)
        local n = 0
        if not ensure(c, sz_sizet) then return end
        for _ = 1, sz_sizet do n = n * 256 + getbyte(c) end
        return n
      end
      ...
      local function goodbyte(c1, c2)
        ...
        if not b1 or not b2 or b1 ~= b2 then return end
        return b1
      end
      ...
      local function goodint(c1, c2)
        ...
        if not i1 or not i2 or i1 ~= i2 then return end
        return i1
      end
      ...
      local function getfunc(c1, c2)
        ...
        if not getstring(c1) or not getstring(c2) then
          bork('bad source name');
          return
        end
        -- linedefined (ignored)
        if not getint(c1) or not getint(c2) then
          bork('bad linedefined');
          return
        end
        -- lastlinedefined (ignored)
        if not getint(c1) or not getint(c2) then
          bork('bad lastlinedefined');
          return
        end
        if not (ensure(c1, 4) and ensure(c2, 4)) then bork('prototype header broken') end
        -- nups (compared)
      ...
      if not cdat then bork('failed to compile compressed result for binary chunk comparison') end
      ...
      if not (ensure(c1, 12) and ensure(c2, 12)) then bork('header broken') end
      ...
      if c1.i ~= c1.len + 1 then
        bork('inconsistent binary chunk1');
        return
      elseif c2.i ~= c2.len + 1 then
        bork('inconsistent binary chunk2');
        return
      end
  ...
  fs = function()
    ...
    function M.read_file(filename, mode)
      ...
      if not handler then
        return nil, fmt('Could not open %s for reading: %s', filename, normalize_io_error(filename, err))
      end

      local content, err = handler:read('*a') -- luacheck: ignore 411
      if not content then return nil, fmt('Could not read %s: %s', filename, normalize_io_error(filename, err)) end

      handler:close()

      if content:sub(1, #UTF8_BOM) == UTF8_BOM then content = content:sub(#UTF8_BOM + 1) end
    ...
    function M.write_file(filename, data, mode)
      ...
      if not handler then
        return nil, fmt('Could not open %s for writing: %s', filename, normalize_io_error(filename, err))
      end

      local _, err = handler:write(data) -- luacheck: ignore 411
      if err then return nil, fmt('Could not write %s: %s', filename, normalize_io_error(filename, err)) end
...
local function dump_parser(srcfl)
  ...
  for i = 1, #localinfo do
    local obj = localinfo[i]
    local msg = '(' .. i .. ') \'' .. obj.name .. '\' decl:' .. obj.decl .. ' act:' .. obj.act .. ' rem:' .. obj.rem
...
local function process_file(srcfl, destfl)
  ...
  if find(dat, '\r\n', 1, 1) or find(dat, '\n\r', 1, 1) then warn.MIXEDEOL = true end

  -- Test source and binary chunk equivalence.
  equiv.init(option, llex, warn)
  equiv.source(z, dat)
  if BIN_EQUIV_AVAIL then equiv.binary(z, dat) end
  ...
  if warn.SRC_EQUIV then
    ...
    if option['opt-locals'] then print('(but no identifier comparisons since --opt-locals enabled)') end
    print()
  end
  ...
  local tabf1, tabf2 = '%-16s%8s%8s%10s%8s%8s%10s', '%-16s%8d%8d%10.2f%8d%8d%10.2f'
  local hl = rep('-', 68)
  print('*** lexer-based optimizations summary ***\n' .. hl)
  print(fmt(tabf1, 'Lexical', 'Input', 'Input', 'Input', 'Output', 'Output', 'Output'))
  print(fmt(tabf1, 'Elements', 'Count', 'Bytes', 'Average', 'Count', 'Bytes', 'Average'))
  print(hl)
  for i = 1, #TTYPES do
    local ttype = TTYPES[i]
    print(fmt(tabf2, ttype, figures(ttype)))
...
local function do_files(fspec)
  for i = 1, #fspec do
    ...
    if #fspec == 1 and option.OUTPUT_FILE then destfl = option.OUTPUT_FILE end
    if srcfl == destfl then die('output filename identical to input filename') end
...
local function main()
  ...
  if option['opt-binequiv'] and not BIN_EQUIV_AVAIL then die('--opt-binequiv is available only for PUC Lua 5.1!') end
  if #fspec > 0 then
    if #fspec > 1 and option.OUTPUT_FILE then die('with -o, only one source file can be specified') end
    do_files(fspec)
    return true
  else
    die('nothing to do!')
...
if not main() then die('Please run with option -h or --help for usage information') end
